...
# Launch this script from console ( $ chmod +x ingress-ice.sh&&./ingress-ice.sh )
FILE="$HOME/PAF/PAF_ScrapIngress/Script/.ingress-ice-nord.conf"
...
launch() {
  ...
  then
    ...
    if [ ! $LOOP ]
    then
      $PHANTOMJS "$SCRIPT_HOME/ingress-ice-master/ice/ice.js" $ARGS; exit;
    else
      while :; do
        $PHANTOMJS "$SCRIPT_HOME/ingress-ice-master/ice/ice.js" $ARGS
      done
    fi
  else
    if [ ! $TEXT ]; then
