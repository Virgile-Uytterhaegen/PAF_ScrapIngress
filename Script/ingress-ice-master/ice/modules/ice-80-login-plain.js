...
function afterPlainLogin() {
  page.open(config.area, function() {
    if (!isSignedIn()) {
      announce('Something went wrong. Your login '+ config.login + ' Please, sign in to Google via your browser and restart ICE. Don\'t worry, your Ingress account will not be affected.');
      quit();
    }
    window.setTimeout(function() {
      storeCookies();
